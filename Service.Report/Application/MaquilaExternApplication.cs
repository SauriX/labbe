    ...
    public class MaquilaExternApplication : BaseApplication, IMaquilaExternApplication
    {
        ...
        public async Task<byte[]> DownloadReportPdf(ReportFilterDto filter)
        {
            ...
            var data = requestData.Select(x => new Dictionary<string, object>
            {
                ...
                { "Children", new List<Dictionary<string, object>> { { new Dictionary<string, object> { { "Clave", x.ClaveEstudio }, { "Estudio", x.NombreEstudio }, { "Estatus", x.Estatus }, { "Maquila", $"Maquila {x.Maquila.ToUpper()}" } } } } },
                { "Edad", x.Edad},
                { "Sexo", x.Sexo},
            ...
            {
                NombreReporte = "Relación Estudios con Maquila Externa por Paciente",
